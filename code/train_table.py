...
# 计算各项评估指标
def calculate_metrics(true, pred, probs):
    """计算各项评估指标"""
    return {
...
# 训练模块（五折交叉验证）
# ======================
def train_with_cv(config, X_train_all, y_train_all, output_csv_path, model_output_path):
    # 初始化最佳模型
    best_model = None
    best_auc = 0
    ...
    random_state = 42
    # 创建 ShuffleSplit 对象
    ss = ShuffleSplit(n_splits=config['cv'], test_size=0.2, random_state=random_state)

    # 进行五次划分
    for fold, (train_idx, val_idx) in enumerate(ss.split(X_train_all)):
        print(f"\n=== Fold {fold+1}/5 ===")
        ...
        log_file = f"{output_csv_path}/training_log_{fold}.csv"
        model_output_dir = os.path.join(model_output_path, f"fold_{fold+1}")
        ...
        train_dataset = TensorDataset(
            torch.tensor(
                X_train[:, : len(config["categorical_cols"])], dtype=torch.long
            ),
            torch.tensor(
                X_train[:, len(config["categorical_cols"]) :], dtype=torch.float32
            ),
            torch.tensor(y_train, dtype=torch.long),
        )

        val_dataset = TensorDataset(
            torch.tensor(X_val[:, : len(config["categorical_cols"])], dtype=torch.long),
            torch.tensor(
                X_val[:, len(config["categorical_cols"]) :], dtype=torch.float32
            ),
        ...
        for epoch in range(config["num_epochs"]):
            model.train()  # 训练步骤
            train_loss, train_preds, train_probs, train_labels = [], [], [], []
            for batch_cat, batch_cont, batch_y in train_loader:
                ...
                train_preds.extend(train_pred.tolist())
                train_probs.extend(torch.softmax(outputs, dim=1)[:, 1].detach().cpu().numpy().tolist())
            ...
            # 验证步骤
            model.eval()
            val_loss, val_preds, val_probs, val_labels = [], [], [], []
            ...
            with torch.no_grad():
                for batch_cat, batch_cont, batch_y in val_loader:
                    ...
                    val_probs.extend(torch.softmax(outputs, dim=1)[:, 1].detach().cpu().numpy().tolist())
            ...
            print(
                f"fold: {fold + 1}, epoch: {epoch+1}, train_loss: {np.mean(train_loss)}, val_loss: {np.mean(val_loss)}"
            )

            # 保存最佳模型
            if val_metrics["auc"] > best_auc:
                best_auc = val_metrics["auc"]
                torch.save(model.state_dict(), f"{model_output_path}/best_model.pth")

            if epoch % config["save_split"] == 0:
                torch.save(
                    model.state_dict(),
                    os.path.join(model_output_dir, f"epoch{epoch+1}.pth"),
                )
...
def evaluate_and_save(
    ...
    with torch.no_grad():
        ...
        preds = np.argmax(outputs.tolist(), axis=1)
...
def main():
    ...
    # 对训练集进行训练, 训练过程中需要使用csv保存epoch,train_loss,val_loss,train_acc,val_acc,train_reecall,val_recall,train_auc,val_auc,train_dice,val_dice
    # 使用五折交叉验证
    print(" -------------- 五折交叉验证 -------------- ")
    output_csv_path = f"tab_results_11_16/csvs"
    model_output_path = f"tab_results_11_16/checkpoints"
    os.makedirs(output_csv_path, exist_ok=True)
    os.makedirs(model_output_path, exist_ok=True)
    train_with_cv(
        config, X_train_all, y_train_all, output_csv_path, model_output_path
    )
    print(" --------------------------------------------- ")

    # 利用训练好的模型, 对测试集进行测试, 将dice,acc,recall,auc值保存到txt中, 将ROC图像保存
    print(" -------------- 模型测试 -------------- ")
    output_test_txt_path = "tab_results_11_16/test_result.txt"
    output_test_roc_path = "tab_results_11_16/test_roc.png"
    ...
    best_model.load_state_dict(torch.load(f"{model_output_path}/best_model.pth", map_location=device))
    evaluate_and_save(
