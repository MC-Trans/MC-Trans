...
class FusionDataset(Dataset):
    def __init__(self, data_list, labels, target_shape=(128, 128, 128), transform=None, cache=False):
        ...
        self.id_to_categorical, self.id_to_continuous = self.get_table("data/train_table.xlsx")

    def get_table(self, excel_path):
        # 定义分类列和连续列（需要根据实际数据调整）
        categorical_cols = config["categorical_cols"]
        continuous_cols = config["continuous_cols"]

        df_train = pd.read_excel(
            excel_path, sheet_name="训练集&验证集"
        )  # 读取训练集&验证集的数据
        df_test = pd.read_excel(excel_path, sheet_name="测试集")  # 读取测试集
        ...
        for _, row in full_df.iterrows():
            ...
            # 分类特征（已编码）
            categorical = row[categorical_cols].to_numpy(dtype=int)
            
            # 连续特征（已归一化）
            continuous = row[continuous_cols].to_numpy(dtype=float)
            
            id_to_categorical[sample_id] = categorical
            id_to_continuous[sample_id] = continuous
        
        return id_to_categorical, id_to_continuous
    ...
    def __getitem__(self, idx):
        ...
        id = int(os.path.basename(self.data_list[idx]).replace(".nrrd", ""))
        categorical = self.id_to_categorical[id]
        continuous = self.id_to_continuous[id]
        # 数据增强
        if self.transform:
            data = self.transform(data)

        return data, categorical, continuous, label
...
if __name__ == "__main__":
    ...
    test_dataloader = get_test_dataloader("data/train_table.xlsx")

    # 验证数据加载
    for batch in train_loader:
        data, categorical, continuous, label = batch
        print(f"训练批次维度: {data.shape}, 标签: {label}")
        print(f"连续特征维度: {continuous.shape}, 类别特征维度: {categorical.shape}")
        break
